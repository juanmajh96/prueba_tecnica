...
void main() {
  ...
  DeleteProductImpl _usecases;
  Product _product;
  setUp(() {
    _product = Product();
    _repositories = CartRepositoriesMock();
    _usecases = DeleteProductImpl(_repositories);
  });

  test('It should return true if It work good', () async {
    when(_repositories.deleteProduct(any)).thenAnswer(
    ...
    verify(_repositories.deleteProduct(_product)).called(1);
  });

  test('It should return CartError if It not work good ', () {
    when(_repositories.deleteProduct(any)).thenThrow(CartError());
